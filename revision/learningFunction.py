...
def learn(starting_genome, duration=2000, size=2, window_size=4000, stepsize=0.1, init_flux=2.75, max_flux=10,
          ...
          verbose=1.00, generator_type='RPG', neuron_configuration=[0], tolerance=None, tracking_parameters=None, filename = None, folderName=None,
          track_fitness=False, print_done=False, trial=0, csv_name=None, genome_num=None):
    ...
    start_fitness = fitnessFunction(starting_genome, N=size, generator_type=generator_type, configuration=neuron_configuration)
    end_fitness = fitnessFunction(learner.recoverParameters(), N=size, generator_type=generator_type, configuration=neuron_configuration, verbose=verbose)
    ...
    if csv_name:
        ...
        elements = {"start_fit":start_fitness,"end_fit":end_fitness,
                    "generator":generator_type, "configuration":neuron_configuration,
                    "init_flux": init_flux,
                    "min_period": min_period, "max_period":max_period, "window_size":window_size,
                    "genome_num":genome_num
                    }
        append_dict_as_row(f"./data/{csv_name}", field_names = elements.keys(), elements=elements)
    ...
    if log_data:
        ...
        if print_done:
            print(f"{generator_type}-{np.round(start_fitness,5)}-{np.round(end_fitness,5)}")
