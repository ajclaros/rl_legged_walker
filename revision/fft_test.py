...
def thresh(grouped, coords, coord_add=0):
    ...
    temp = grouped[np.where(
        (temp[:,-1]>=coords[0])&(temp[:,-1]<=coords[1])
    )]#[:,coords[2]:coords[3]]
    return temp
...
arr= thresh(grouped, (begin_y, add_y, begin_x, add_x), coord_add=data['learning_start']/data['stepsize'])
...
line_6, = ax[0].plot(t, np.mean(arr, axis=0)+arr.std(axis=0), color='y', ls="dotted", label=r'mean $\pm$ std')
...
x_1_slider = Slider(
    ...
    label="x_1",
    valmin=1,
    valmax=data['duration'], valstep=0.1,
    valinit=init_x_1,
    orientation="horizontal"
)
...
def update(val):
    ...
    arr = thresh(grouped, (y_0, y_1, 0, x_1*10), coord_add=data['learning_start']/data['stepsize'])
