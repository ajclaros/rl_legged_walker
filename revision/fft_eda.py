...
def thresh(grouped, coords):
    #temp = grouped[:,coords[2]: coords[3]]
    temp = grouped[:,:coords[3]]

    if temp.size==0:
        return np.zeros(coords[3])
    temp = grouped[np.where((temp[:,-1]>=coords[0])&(temp[:,-1]<=coords[1]))]#[:,coords[2]:coords[3]]
    return temp
...
arr= thresh(grouped, (begin_y, add_y, begin_x, add_x))
line_1 = ax[0].axhline(begin_y)
line_2 = ax[0].axhline(add_y)
#line_3 = ax[0].axvline(begin_x, c='r', ls='dotted')
