...
class LeggedAgent:
    ...
    def stepN(self, stepsize, u, configuration = [0,0,0]):
        force = 0.0
        if len(configuration)==1:
            if (u[configuration[0]] > 0.5):
                self.footstate = 1
                self.omega = 0
                self.forwardForce = 2 * (u[configuration[0]] - 0.5) * MaxLegForce;
                self.backwardForce = 0.0
            else:
                self.footstate = 0
                self.forwardForce = 0.0
                self.backwardForce = 2 * (0.5 - u[configuration[0]]) * MaxLegForce;
