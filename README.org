...
The main CTRNN is an extension and modification of an implementation of a CTRNN CPG oscillation task by Yoder et al 2022
...
- folderName: creates folder if it does not exist within "./data/" to save each trial. Writes params.txt within folder of parameters used for the current experiment.
...
- neuron_configuration: list of size up to length 3 containing the indices of a neuron.
...
- windowed mode moves y_1 with y_0. Change to free mode to freely move y_0 and y_1. Press button to change mode.
